...
SELECT MaNV, HoNV, TenNV, NgaySinh, GioiTinh, DiaChi, SDT, CongViec.TenCV, SoCa, Thuong, NgayTD
...
CREATE VIEW [dbo].[v_BangPhanCa]
AS
SELECT NHANVIEN.MaNV, NHANVIEN.HoNV, NHANVIEN.TenNV, BangPhanCa.MaCa, BangPhanCa.NgayLam
...
-- Danh mục hóa đơn
CREATE VIEW [dbo].[v_HoaDonView] AS
SELECT MaHD, NgayDatHang, MaKH, MaNV, TriGiaHD
FROM HOADON;
GO

-- Danh mục sản phẩm
CREATE VIEW [dbo].[v_DanhSachSP] AS
SELECT *
FROM SANPHAM
GO

-- Danh mục phiếu chi
CREATE VIEW [dbo].[v_DonNhapHangView]
AS
SELECT DNH.MaDonNH, DNH.NgayNH, DNH.MaNCC, DNH.TinhTrang, PC.MaPC, 
	PC.NgayXuatPhieu, CTD.MaNL, CTD.DonGia, CTD.SL, CTD.DonVi, PC.SoTienChi FROM DONNHAPHANG DNH
LEFT JOIN PhieuChi PC ON DNH.MaDonNH = PC.MaDonNH
LEFT JOIN ChiTietDonNhap CTD ON DNH.MaDonNH = CTD.MaDonNH;
GO

-- Danh mục nguyên liệu
CREATE VIEW [dbo].[v_NguyenLieu] AS
SELECT *
FROM NGUYENLIEU
GO
